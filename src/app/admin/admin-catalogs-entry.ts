...
@Component({
  selector: 'app-admin-catalogs-entry',
  standalone: true,
    ...
    <section class="sabr-page admin-entry-page">
      <div class="sabr-card panel entry-panel">
        ...
        <div *ngIf="!loading && clients.length > 0" class="entry-picker">
          <h3>Selecione o cliente para acessar Catalogos</h3>
...
export class AdminCatalogsEntry implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (context?.tenantId) {
      void this.router.navigate([`/t/${context.tenantId}/catalogs`], { replaceUrl: true });
      return;
    }
      ...
      .subscribe({
        next: (response) => {
          ...
          if (response.items.length === 1) {
            ...
            this.tenantContext.set(client.tenantId, client.accountName, client.id);
            void this.router.navigate([`/t/${client.tenantId}/catalogs`], { replaceUrl: true });
